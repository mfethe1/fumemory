...
class NATSClusterManager:
    ...
    def _build_auth_opts(self, node: ClusterNode) -> dict[str, Any]:
        """Resolve authentication connect-options for a specific NATS node.

        LOCAL node — NATS_LOCAL_NKEY_SEED takes priority (the Ed25519 user seed
        generated by infra/local-nats/gen-nkeys.sh for the single-host instance).
        Falls back to NATS_CREDS_FILE / NATS_NKEY_SEED if not set.

        RAILWAY node — NATS_CREDS_FILE or NATS_NKEY_SEED only (Synadia Cloud /
        NGS credentials).  NATS_LOCAL_NKEY_SEED is intentionally ignored for the
        Railway node so that the two clusters can carry different credentials.
        """
        opts: dict[str, Any] = {}

        if self.auth_token:
            opts["token"] = self.auth_token
            return opts
        ...
        if node == ClusterNode.LOCAL:
            local_nkey_seed = os.environ.get("NATS_LOCAL_NKEY_SEED", "").strip() or None
            if local_nkey_seed:
                opts["nkeys_seed"] = local_nkey_seed
                return opts

        if creds_file:
            opts["user_credentials"] = creds_file
        elif nkey_seed:
            opts["nkeys_seed"] = nkey_seed
    ...
    async def _connect_node(self, node: ClusterNode, url: str):
        ...
        try:
            ...
            connect_opts.update(self._build_auth_opts(node))
