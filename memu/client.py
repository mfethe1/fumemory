"""Python clients for the memU API."""
...
class MemUClient:
    """Synchronous client for interacting with a memU server."""
    ...
    def ping(self) -> bool:
        ...
        try:
            health = self.health()
            return health.get("status") == "healthy" or health.get("ok") is True
        except Exception:
            return False
    ...
    def get_recent(
        ...
        """Fetch recent memories with optional filters."""
        params = {"limit": limit}
        if agent_id:
    ...
    def format_for_prompt(self, memories: list[Memory]) -> str:
        ...
        for memory in memories:
            source = f"agent={memory.agent_id}" if memory.agent_id else "system"
            ...
            output.append(
                f'  <memory id="{memory.id}" source="{source}" type="{memory_type}" '
                f'confidence="{memory.confidence}" created_at="{created_at}">'
            )
            output.append(f"    {memory.content.strip()}")
    ...
    def summarize(
        ...
        *,
        ...
        """Get a concise summary of memories related to a topic."""
        prompt = (
            f"Summarize everything we know about '{topic}'. "
            "Be concise and use bullet points. Ignore irrelevant context."
        )
...
class AsyncMemUClient:
    """Async client for interacting with a memU server."""
    ...
    async def get_stats(self) -> dict:
        """Get total memory counts by agent and type."""
        r = await self._client.get("/api/v1/memu/stats")
        r.raise_for_status()
        return r.json()
    ...
    async def delete(self, memory_id: str | UUID) -> None:
        """Delete a memory."""
    ...
    async def chat(
        ...
        *,
        ...
        r = await self._client.post("/chat", json=payload)
        r.raise_for_status()
        return ChatResponse.model_validate(r.json())
    ...
    async def summarize(
        ...
        *,
        ...
        resp = await self.chat(prompt, agent_id=agent_id, context_limit=context_limit)
        return resp.answer
    ...
    async def bulk_import(
        ...
        *,
        ...
        r = await self._client.post("/memories/bulk", json=payload)
        r.raise_for_status()
        return BulkImportResponse.model_validate(r.json())
